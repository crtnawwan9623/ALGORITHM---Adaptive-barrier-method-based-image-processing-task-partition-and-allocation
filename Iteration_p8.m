% future figure 17
% average allocated ratio & average sigle device profits - task size comparison of TPR Tmn Rdn
warning('off');
global D;
global j_tp4;
global tsk;
...
Num_Cf=340;    % for each D, produce Num_Cf config
D=4;

% tsk_size=[2,6,8];
tsk_size=[2,3,4,5,6,7,8,9,10];
for j_tp4=301:Num_Cf
    mainp;
...
%begin plot
            ...
            plot(tsk_size,mean_TPRqg,'r-x',tsk_size,mean_Tmnqg,'b-o',tsk_size,mean_Rdmqg,'k-s','markersize',10,'linewidth',1.5);
            set(gca,'FontSize',14);
            ...
            figure
            plot(tsk_size,mean_TPRdpg,'r-x',tsk_size,mean_Tmndpg,'b-o',tsk_size,mean_Rdmdpg,'k-s','markersize',10,'linewidth',1.5);
            set(gca,'FontSize',14);
            xlabel('Task size (Mbit)','fontsize',14);
            ylabel('Average device value','fontsize',14);
            set(gca,'Fontname','times new Roman');
            leng1=legend('ABM','Timemin','Random');
            set(leng1,'position',[0.154761908912943 0.188492069168696 0.267857137588518 0.203571422894796]);
            grid;
